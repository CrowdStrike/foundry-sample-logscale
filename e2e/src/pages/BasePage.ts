...
export abstract class BasePage {
  ...
  protected async takeScreenshot(filename: string, context: LogContext = {}): Promise<void> {
    try {
      ...
      this.logger.warn(`Failed to take screenshot: ${filename} - ${errorMessage}`);
    }
  }
  ...
  protected async elementExists(
    ...
    try {
      ...
      this.logger.debug(`Element not found in expected state '${state}': ${typeof locator === 'string' ? locator : 'locator'}`);
      return false;
    }
  }
